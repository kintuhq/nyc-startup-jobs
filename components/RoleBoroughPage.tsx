...
import { MapPin, Mail, Briefcase, Building } from "lucide-react"
...
interface RoleBoroughPageProps {
  role: string
...
export default function RoleBoroughPage({
  role,
  roleDisplayName,
  borough,
  boroughDisplayName,
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`/api/jobs?${params.toString()}`, {
        next: {
          revalidate: 60,
          tags: ['jobs', `jobs-${borough}`, `jobs-role-${role.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}`]
        },
        cache: 'force-cache'
      })
      if (response.ok) {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSubscriptionMessage(`Thanks! You'll receive notifications about new ${roleDisplayName} positions in ${boroughDisplayName}.`)
        setEmail("")
      ...
      <section className="py-16 px-4">
        <div className="mx-auto text-center" style={{ maxWidth: "1240px" }}>
          ...
          <h1 className="text-5xl font-bold text-foreground text-balance mb-4">
            <span className="bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">{roleDisplayName}</span> Jobs in {boroughDisplayName}
          </h1>
              ...
              {!showEmailInput ? (
                <>
                  ...
                  >
                    ...
                    <span className="text-gray-500 text-xs sm:text-sm font-medium">
                      Get {roleDisplayName} jobs in {boroughDisplayName}
                    </span>
                  </div>
                ...
                <>
                  ...
                  >
                    ...
                    <input
                      type="email"
                      placeholder={`${roleDisplayName} jobs in ${boroughDisplayName}`}
                      value={email}

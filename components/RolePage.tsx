...
export default function RolePage({ role, roleDisplayName, description, metaTitle }: RolePageProps) {
  ...
  const fetchJobs = async () => {
    try {
      const response = await fetch(`/api/jobs?role=${encodeURIComponent(role)}&limit=20`, {
        next: {
          revalidate: 60,
          tags: ['jobs', `jobs-role-${role.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}`]
        },
        cache: 'force-cache'
      })
      if (response.ok) {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSubscriptionMessage(`Thanks! You'll receive notifications about new ${roleDisplayName} positions.`)
        setEmail("")
              ...
              {!showEmailInput ? (
                <>
                  ...
                  >
                    ...
                    <span className="text-gray-500 text-xs sm:text-sm font-medium">
                      Get {roleDisplayName} job alerts
                    </span>
                  </div>
                ...
                <>
                  ...
                  >
                    ...
                    <input
                      type="email"
                      placeholder={`${roleDisplayName} job alerts`}
                      value={email}
      ...
      <section className="py-8 px-4">
        <div className="mx-auto" style={{ maxWidth: "1240px" }}>
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-2xl font-bold text-foreground">{roleDisplayName} Opportunities</h3>
          ...
          {!isLoading && jobs.length === 0 && (
            <div className="text-center py-12">
              <p className="text-muted-foreground text-lg">No {roleDisplayName} opportunities yet.</p>
              <p className="text-muted-foreground">Subscribe above to get notified when new {roleDisplayName} positions are posted!</p>
            </div>
          )}
        </div>
      </section>
